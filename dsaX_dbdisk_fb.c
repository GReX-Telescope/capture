...
void send_string(char *string) /* includefile */
{
  ...
  fwrite(string, sizeof(char), len, fout);
  /*fprintf(stderr,"%s\n",string);*/
}
...
{
  send_string(name);
  fwrite(&floating_point,sizeof(float),1,fout);
  /*fprintf(stderr,"%f\n",floating_point);*/
}
...
{
  send_string(name);
  fwrite(&double_precision,sizeof(double),1,fout);
  /*fprintf(stderr,"%f\n",double_precision);*/
}
...
{
  send_string(name);
  fwrite(&integer,sizeof(int),1,fout);
  /*fprintf(stderr,"%d\n",integer);*/
}
...
{
  send_string(name);
  fwrite(&integer,sizeof(long),1,fout);
  /*fprintf(stderr,"%ld\n",integer);*/
}
...
int main (int argc, char *argv[]) {
  ...
  int nbits = 16;
  int nifs = 1;
  char  source_name[20] = "J0000+1111";
  double psrdm=0.;
  ...
  log = multilog_open ("dsaX_dbdisk_fb", 0);
  ...
  while (!observation_complete) {
    ...
    sprintf(fnam,"/home/user/sample_data/fl_%d.filterbank",wseq);
    fout = fopen(fnam,"wb");
